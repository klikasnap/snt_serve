...
class CuRs:
    ...
    def rjson(  # noqa: C901
        ...
        print(CONSTANTS_YML.get("http", {}).get("code", {}).get(status_code, None))

        if error is None:
            error = DEF_ERR.get(c0, False)


        if rci := CONSTANTS_YML.get("http", {}).get("code", {}).get(status_code, None):
            rci = deepcopy(rci)
            ci = {}
            for i in STAT_PRIO:
            ...
            gi = {}
            rgi = deepcopy(CONSTANTS_YML["http"]["code"][c0])
            for i in STAT_PRIO:
