...
class CustomResponse:
    ...
    def rjson(  # noqa: C901
        ...
        if rci := CONSTANTS_YML["http"]["code"].get(str(status_code), None):
            ...
            if (status_code < 100) or (status_code > 799):
                message = f"Code {status_code} is either below 100 or 511 and is therefore not acceptable."
            else:
                message = f"Code {status_code} does not exist."
            status_code = 400
            c0 = 4
